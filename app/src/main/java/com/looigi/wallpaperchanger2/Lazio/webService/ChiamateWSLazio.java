package com.looigi.wallpaperchanger2.Lazio.webService;
...
import com.looigi.wallpaperchanger2.Lazio.Strutture.StrutturaAnni;
import com.looigi.wallpaperchanger2.Lazio.Strutture.StrutturaCalendario;
import com.looigi.wallpaperchanger2.Lazio.Strutture.StrutturaClassifica;
import com.looigi.wallpaperchanger2.Lazio.Strutture.StrutturaCompetizioni;
...
import com.looigi.wallpaperchanger2.Lazio.Strutture.StrutturaMarcatori;
...
import com.looigi.wallpaperchanger2.Lazio.Strutture.StrutturaStati;
import com.looigi.wallpaperchanger2.Lazio.UtilityLazio;
import com.looigi.wallpaperchanger2.Lazio.VariabiliStaticheLazio;
import com.looigi.wallpaperchanger2.Lazio.adapters.AdapterListenerAllenatori;
import com.looigi.wallpaperchanger2.Lazio.adapters.AdapterListenerCalendario;
import com.looigi.wallpaperchanger2.Lazio.adapters.AdapterListenerClassifica;
import com.looigi.wallpaperchanger2.Lazio.adapters.AdapterListenerFonti;
import com.looigi.wallpaperchanger2.Lazio.adapters.AdapterListenerGiocatori;
import com.looigi.wallpaperchanger2.Lazio.adapters.AdapterListenerMarcatori;
import com.looigi.wallpaperchanger2.Lazio.adapters.AdapterListenerMercato;
import com.looigi.wallpaperchanger2.Lazio.adapters.AdapterListenerRuoli;
import com.looigi.wallpaperchanger2.Lazio.adapters.AdapterListenerSquadre;
import com.looigi.wallpaperchanger2.Lazio.adapters.AdapterListenerStati;
...
import com.looigi.wallpaperchanger2.Lazio.api_football.strutture.Allenatori.Coach;
import com.looigi.wallpaperchanger2.Lazio.api_football.strutture.Giocatori.Cards;
import com.looigi.wallpaperchanger2.Lazio.api_football.strutture.Giocatori.Games;
import com.looigi.wallpaperchanger2.Lazio.api_football.strutture.Giocatori.GiocatoriPartita;
import com.looigi.wallpaperchanger2.Lazio.api_football.strutture.Giocatori.Goals;
import com.looigi.wallpaperchanger2.Lazio.api_football.strutture.Giocatori.PlayerStatistics;
import com.looigi.wallpaperchanger2.Lazio.api_football.strutture.Giocatori.Statistics;
import com.looigi.wallpaperchanger2.Lazio.api_football.strutture.Partita.Partita;
import com.looigi.wallpaperchanger2.Lazio.api_football.strutture.Partite.FixtureData;
...
public class ChiamateWSLazio implements TaskDelegateLazio {
    ...
    private String idSquadraDaAggiungereCasa;
    private String SquadraDaAggiungereCasa;
    private String CognomiDaAggiungereCasa = "";
    private String NomiDaAggiungereCasa = "";
    private String idSDaAggiungereCasa = "";
    private String idSquadraDaAggiungereFuori;
    private String SquadraDaAggiungereFuori;
    private String CognomiDaAggiungereFuori = "";
    private String NomiDaAggiungereFuori = "";
    private String idSDaAggiungereFuori = "";
    ...
    public void AggiungeTuttiGliAllenatori() {
        ...
        if (VariabiliStaticheLazio.getInstance().getSquadre() == null) {
            ...
            UtilitiesGlobali.getInstance().ImpostaSpinner(
                    context,
                    VariabiliStaticheLazio.getInstance().getSpnSquadreGioc(),
                    righePerSpinner,
                    ""
            );
            UtilitiesGlobali.getInstance().ImpostaSpinner(
                    context,
                    VariabiliStaticheLazio.getInstance().getSpnSquadreAll(),
                    righePerSpinner,
                    ""
            );
    ...
    public void SalvaDettaglio() {
        ...
        int goalCasa = 0;
        if (datiPartita.score != null && datiPartita.score.fulltime != null && datiPartita.score.fulltime.home != null) {
            goalCasa = datiPartita.score.fulltime.home;
        }
        int goalFuori = 0;
        if (datiPartita.score != null && datiPartita.score.fulltime != null && datiPartita.score.fulltime.away != null) {
            goalFuori = datiPartita.score.fulltime.away;
        }
        ...
        idSquadraDaAggiungereCasa = String.valueOf(idCasa);
        SquadraDaAggiungereCasa = Casa;
        CognomiDaAggiungereCasa = "";
        NomiDaAggiungereCasa = "";
        idSDaAggiungereCasa = "";
        idSquadraDaAggiungereFuori = String.valueOf(idFuori);
        SquadraDaAggiungereFuori = Fuori;
        CognomiDaAggiungereFuori = "";
        NomiDaAggiungereFuori = "";
        idSDaAggiungereFuori = "";
        ...
        for (int i = 0; i < 2; i++) {
            if (!g.response.isEmpty() && g.response.get(i) != null) {
                for (PlayerStatistics player : g.response.get(i).players) {
                    ...
                    int idGiocatoreApiFootball = player.player.id;
                    ...
                    if (i == 0) {
                        if (VariabiliStaticheApiFootball.getInstance().getGiocatoriCasaPS() != null) {
                            for (StrutturaGiocatori sg : VariabiliStaticheApiFootball.getInstance().getGiocatoriCasaPS()) {
                                ...
                                if (sg.getIdApiFootball() == idGiocatoreApiFootball) {
                    ...
                    if (idGiocatore == -1) {
                        ...
                        if (i == 0) {
                            CognomiDaAggiungereCasa += Cognome.trim() + ";";
                            NomiDaAggiungereCasa += NomeGiocatore.trim() + ";";
                            idSDaAggiungereCasa += id.trim() + ";";
                        } else {
                            CognomiDaAggiungereFuori += Cognome.trim() + ";";
                            NomiDaAggiungereFuori += NomeGiocatore.trim() + ";";
                            idSDaAggiungereFuori += id.trim() + ";";
                        }

                        idGiocatore = Integer.parseInt(id);
                    }
    ...
    private void fSalvaDettaglio(String result) {
        ...
        if (!ritorno) {
            ...
            if (!CognomiDaAggiungereCasa.isEmpty()) {
                String Urletto = "AggiungeGiocatori?" +
                        "idAnno=" + VariabiliStaticheApiFootball.getInstance().getIdAnnoScelto() +
                        "&idSquadra=" + idSquadraDaAggiungereCasa +
                        "&Cognomi=" + CognomiDaAggiungereCasa +
                        "&Nomi=" + NomiDaAggiungereCasa +
                        "&idGiocApiFootball=" + idSDaAggiungereCasa +
                        "&Squadra=" + SquadraDaAggiungereCasa;
                // "&Loghi=" + Loghi;

                TipoOperazione = "AggiungeGiocatori";
                if (!CognomiDaAggiungereFuori.isEmpty()) {
                    AggiuntaGiocatori = "MANCANOFUORI";
                } else {
                    AggiuntaGiocatori = "BASTA";
                }
            ...
            if (!AggiuntaGiocatori.isEmpty()) {
                if (AggiuntaGiocatori.equals("BASTA")) {
                    ...
                    if (AggiuntaGiocatori.equals("MANCANOFUORI")) {
                        String Urletto = "AggiungeGiocatori?" +
                                "idAnno=" + VariabiliStaticheApiFootball.getInstance().getIdAnnoScelto() +
                                "&idSquadra=" + idSquadraDaAggiungereFuori +
                                "&Cognomi=" + CognomiDaAggiungereFuori +
                                "&Nomi=" + NomiDaAggiungereFuori +
                                "&idGiocApiFootball=" + idSDaAggiungereFuori +
                                "&Squadra=" + SquadraDaAggiungereFuori;
                        // "&Loghi=" + Loghi;
    ...
    private void fRitornaStati(String result) {
        ...
        if (!ritorno) {
            ...
            UtilitiesGlobali.getInstance().ImpostaSpinner(
                    context,
                    VariabiliStaticheLazio.getInstance().getSpnStati(),
                    righePerSpinner,
                    ""
            );
            // ArrayAdapter<String> adapter = new ArrayAdapter<String>
    ...
    private void fRitornaRuoli(String result) {
        ...
        if (!ritorno) {
            ...
            UtilitiesGlobali.getInstance().ImpostaSpinner(
                    context,
                    VariabiliStaticheLazio.getInstance().getSpnRuolo(),
                    righePerSpinner,
                    ""
            );
            // ArrayAdapter<String> adapter = new ArrayAdapter<String>
    ...
    private void fRitornaCompetizioni(String result) {
        ...
        if (!ritorno) {
            ...
            ArrayAdapter<String> adapter = UtilitiesGlobali.getInstance().ImpostaSpinner(
                    context,
                    VariabiliStaticheLazio.getInstance().getSpnCompetizioni(),
                    CompetizioniPerSpinner,
                    ""
            );
    ...
    private void fRitornaAnni(String result) {
        ...
        if (!ritorno) {
            ...
            UtilitiesGlobali.getInstance().ImpostaSpinner(
                    context,
                    VariabiliStaticheLazio.getInstance().getSpnAnni(),
                    AnniPerSpinner,
                    PrimoAnno
            );
