...
public class ProviderPlayer extends AppWidgetProvider {
	private static final String NomeMaschera = "Provider_Player";

	@Override
	public void onUpdate(Context context, AppWidgetManager appWidgetManager,
						 int[] appWidgetIds) {
		UtilityPlayer.getInstance().ScriveLog(context, NomeMaschera, "onUpdate ProviderPlayer 1");

		VariabiliStatichePlayer.getInstance().setContext(context);
		...
		for (int appWidgetId : appWidgetIds) {
			...
			Intent launchMain = new Intent(context, AprePlayerDaWidget.class);
			PendingIntent pendingMainIntent = PendingIntent.getActivity(
					context,
					0,
					launchMain,
					PendingIntent.FLAG_IMMUTABLE);
			views.setOnClickPendingIntent(R.id.imgPlayer, pendingMainIntent);
	...
	private static void setRemoteAdapter(Context context, @NonNull final RemoteViews views) {
		views.setRemoteAdapter(R.id.imgPlayer,
				new Intent(context, AprePlayerDaWidget.class));
	}
